...
const VALID_PLATFORMS = ["META", "GOOGLE", "TIKTOK", "INSTAGRAM", "YOUTUBE", "LAINNYA"];
...
export async function GET(request: NextRequest) {
  ...
  const performances = await prisma.marketingAd.findMany({
    where,
    orderBy: { tanggal: "desc" },
    include: {
      adv: { select: { name: true, teamType: true } },
    },
  });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const targetPlatform: any = VALID_PLATFORMS.includes(platform) ? platform : "META";
